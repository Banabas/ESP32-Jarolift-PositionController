...
enum DeviceType { DEV_TEXT, DEV_BTN, DEV_SHUTTER, DEV_BTN_SHUTTER };
enum statType { TYP_STATUS, TYP_INFO, TYP_WIFI, TYP_ETH, TYP_SYSINFO, TYP_CMD_BTN, TYP_SHUTTER, TYP_GROUP, TYP_SHUTTER_BTN };
...
void mqttHaConfig(statType statType, const char *name, const char *deviceClass, const char *component, const char *unit, const char *valueTemplate,
  ...
  if (devType == DEV_SHUTTER) {
    ...
    if (statType == TYP_GROUP) {
      // Groups: no position feedback → optimistic mode
      doc["optimistic"] = true;
    ...
    doc["ent_cat"] = "config";
  } else if (devType == DEV_BTN_SHUTTER) {
    // Shade button for individual shutter
    sprintf(cmdTopic, "%s/cmd/shutter/%i", statePrefix, devCfg.num1);
    doc["cmd_t"]         = cmdTopic;
    doc["payload_press"] = "SHADE";
    // Friendly name: "<shutter name> Shade"
    char shadeFriendly[80];
    snprintf(shadeFriendly, sizeof(shadeFriendly), "%s Shade", devCfg.char1);
    doc["name"] = shadeFriendly;
...
void mqttDiscoverySetup(bool reset) {
  ...
  // Shade buttons for each enabled shutter
  for (int i = 0; i < 16; i++) {
    if (config.jaro.ch_enable[i]) {
      char shadeName[32];
      snprintf(shadeName, sizeof(shadeName), "shutter%d_shade", i + 1);
      mqttHaConfig(TYP_SHUTTER_BTN, shadeName, NULL, "button", NULL, NULL,
                   "mdi:roller-shade", DEV_BTN_SHUTTER, shutterPar(i + 1, config.jaro.ch_name[i]));
    }
  }
